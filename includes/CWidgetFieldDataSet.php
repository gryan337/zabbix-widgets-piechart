...
namespace Modules\RMEPieChart\Includes;
		...
		if ($total_item_count > 0) {
			foreach ($value as $data) {
				if ($data['dataset_aggregation'] !== AGGREGATE_NONE || $data['itemname_aggregation'] !== AGGREGATE_NONE) {
					return [_('Cannot set "Data set aggregation" or "Item name aggregation" when item with type "Total" is added to the chart.')];
				}
			}
		}
