...
class CSVGPieRME {
	...
	constructor(padding, config) {
		...
		this.#radius_inner = this.#config.draw_type === CSVGPieRME.DRAW_TYPE_PIE
			? 0
		...
		this.#svg = d3.create('svg:svg').attr('class', CSVGPieRME.ZBX_STYLE_CLASS);
	...
	setValue({sectors, all_sectorids, total_value}) {
		...
		if (total_value.value > 0) {
			this.#container
				.selectAll(`.${CSVGPieRME.ZBX_STYLE_ARC_NO_DATA_OUTER}, .${CSVGPieRME.ZBX_STYLE_ARC_NO_DATA_INNER}`)
				.style('display', 'none');
		}
		...
		this.#arcs_container
			.selectAll(`.${CSVGPieRME.ZBX_STYLE_ARC_CONTAINER}`)
			.data(this.#pieGenerator(was), key)
			.join('svg:g')
			.attr('class', CSVGPieRME.ZBX_STYLE_ARC_CONTAINER)
			.attr('data-hintbox', 1)
			...
			.each((d, index, nodes) => {
				...
				d3.select(sector)
					.selectAll(`.${CSVGPieRME.ZBX_STYLE_ARC_PLACEHOLDER}`)
					.data((d) => [d])
					.join('svg:path')
					.attr('class', CSVGPieRME.ZBX_STYLE_ARC_PLACEHOLDER)
					.attr('d', (d) => this.#arcGenerator(d))
					.each((d, index, nodes) => nodes[index]._current = d);

				if (this.#config.stroke > 0) {
					d3.select(sector)
						.selectAll(`.${CSVGPieRME.ZBX_STYLE_ARC_STROKE}`)
						.data((d) => [d])
						.join('svg:path')
						.attr('class', CSVGPieRME.ZBX_STYLE_ARC_STROKE)
						.style('transition', this.#sectors_old.length > 0
							? `fill ${CSVGPieRME.ANIMATE_DURATION_WHOLE}ms linear`
							: ''
						)
						.each((d, index, nodes) => nodes[index]._current = d);
				}
				...
				d3.select(sector)
					.selectAll(`.${CSVGPieRME.ZBX_STYLE_ARC}`)
					.data((d) => [d])
					.join('svg:path')
					.attr('class', CSVGPieRME.ZBX_STYLE_ARC)
					.style('transition', this.#sectors_old.length > 0
						? `fill ${CSVGPieRME.ANIMATE_DURATION_WHOLE}ms linear`
						: ''
					)
		...
		if (this.#config.space > 0) {
			this.#space_container
				.selectAll(`.${CSVGPieRME.ZBX_STYLE_SPACE}`)
				.data(this.#pieGenerator(was), key)
				.join('svg:line')
				.attr('class', CSVGPieRME.ZBX_STYLE_SPACE)
				.attr('x1', 0)
				.attr('x2', 0)
		...
		this.#rendered_promise = this.#arcs_container
			.selectAll(`.${CSVGPieRME.ZBX_STYLE_ARC_PLACEHOLDER}, .${CSVGPieRME.ZBX_STYLE_ARC_STROKE}, .${CSVGPieRME.ZBX_STYLE_ARC}`)
			.transition()
			.duration(CSVGPieRME.ANIMATE_DURATION_WHOLE)
			.attrTween('d', (d, index, nodes) => {
				...
				return t => {
					...
					if (this.#config.stroke > 0 && _this.classList.contains(CSVGPieRME.ZBX_STYLE_ARC)) {
			...
			.end()
			.then(() => {
				const sectors = this.#arcs_container.selectAll(`.${CSVGPieRME.ZBX_STYLE_ARC_CONTAINER}`).nodes();
		...
		this.#arcs_container
			...
			.exit()
			.transition()
			.delay(CSVGPieRME.ANIMATE_DURATION_WHOLE)
			.duration(0)
			.remove()
			.end()
			.then(() => {
	...
	#createContainers() {
		...
		this.#arcs_container = this.#container
			.append('svg:g')
			.attr('class', CSVGPieRME.ZBX_STYLE_ARCS);

		if (this.#config.space > 0) {
			this.#space_container = this.#container
				.append('svg:g')
				.attr('class', CSVGPieRME.ZBX_STYLE_SPACE_CONTAINER);
		}

		this.#container
			.append('svg:circle')
			.attr('class', CSVGPieRME.ZBX_STYLE_ARC_NO_DATA_OUTER)
			.attr('r', this.#radius_outer);

		if (this.#config.draw_type === CSVGPieRME.DRAW_TYPE_DOUGHNUT) {
			this.#container
				.append('svg:circle')
				.attr('class', CSVGPieRME.ZBX_STYLE_ARC_NO_DATA_INNER)
				.attr('r', this.#radius_inner);

			let total_value_width = this.#radius_inner * 2;
			total_value_width -= total_value_width / CSVGPieRME.TOTAL_VALUE_PADDING;

			if (this.#config.total_value?.show) {
				this.#total_value_container = this.#container
					.append('svg:foreignObject')
					.attr('class', CSVGPieRME.ZBX_STYLE_TOTAL_VALUE)
		...
		this.#no_data_container = this.#container
			.append('svg:text')
			.attr('class', CSVGPieRME.ZBX_STYLE_TOTAL_VALUE_NO_DATA)
			.style('font-weight', this.#config.total_value?.is_bold ? 'bold' : '')
			.text(t('No data'));
	}
	...
	#positionValue() {
		const getAutoFontSize = (text, default_size, font_weight) => {
			...
			available_width -= available_width / CSVGPieRME.TOTAL_VALUE_PADDING;
			...
			const max_height = available_width * CSVGPieRME.TEXT_BASELINE;

			const normal_height = Math.min(default_height, max_height) * .875;
			const min_height = CSVGPieRME.TOTAL_VALUE_HEIGHT_MIN;
		...
		if (this.#sectors_new.length > 0) {
			...
			if (this.#config.total_value?.show) {
				...
				if (this.#config.total_value.is_custom_size) {
					...
					if (this.#scale > 0) {
						...
						this.#total_value_font_size = getAutoFontSize(
							this.#total_value_text, CSVGPieRME.TOTAL_VALUE_HEIGHT_MIN, font_weight
						);
					}
					else {
				...
				this.#total_value_container
					.attr('y', -this.#total_value_font_size / 2 / CSVGPieRME.TEXT_BASELINE)
					.attr('height', `${this.#total_value_font_size / CSVGPieRME.TEXT_BASELINE}px`)
					.style('line-height', `${this.#total_value_font_size / CSVGPieRME.TEXT_BASELINE}px`)
			...
			if (this.#config.total_value?.show) {
				...
				if (this.#config.total_value.is_custom_size) {
					this.#no_data_container
						.attr('y', this.#config.total_value.size / 2 * 10 * CSVGPieRME.TEXT_BASELINE)
					...
					const text_width = this.#getMeasuredTextWidth(
						this.#no_data_container.text(), CSVGPieRME.TOTAL_VALUE_SIZE_DEFAULT, font_weight
					);

					let text_scale = this.#radius_inner * 2 / text_width;
					text_scale -= text_scale / CSVGPieRME.TOTAL_VALUE_PADDING;

					this.#no_data_container
						.attr('transform', `scale(${text_scale})`)
						.attr('y', CSVGPieRME.TOTAL_VALUE_SIZE_DEFAULT / 2 * CSVGPieRME.TEXT_BASELINE)
						.style('font-size', `${CSVGPieRME.TOTAL_VALUE_SIZE_DEFAULT}px`);
				}
			}
			else {
				this.#no_data_container
					.attr('y', CSVGPieRME.TOTAL_VALUE_SIZE_DEFAULT / 2 * 10 * CSVGPieRME.TEXT_BASELINE)
					.style('font-size', `${CSVGPieRME.TOTAL_VALUE_SIZE_DEFAULT * 10}px`);
			}
		}
	}
	...
	#getHint(sector) {
		...
		hint.append('span')
			...
			.text(() => {
				...
				text += ` (${sector.percent_of_total.toFixed(2)} %)`;
	 ...
	 */
	#onMouseMove(e) {
		const sector = e.target.closest(`.${CSVGPieRME.ZBX_STYLE_ARC_CONTAINER}`);
	...
	#popOut(sector) {
		...
		sector_d3
			.transition()
			.duration(CSVGPieRME.ANIMATE_DURATION_POP_OUT)
		...
		sector_d3
			.select(`.${CSVGPieRME.ZBX_STYLE_ARC_PLACEHOLDER}`)
			.transition()
			.duration(CSVGPieRME.ANIMATE_DURATION_POP_OUT)
			.attr('transform', `scale(${1 - this.#pop_out_distance})`);
	...
	#popIn(sector) {
		if (!sector.dataset.expanded) {
			...
			sector_d3
				.transition()
				.duration(CSVGPieRME.ANIMATE_DURATION_POP_IN)
